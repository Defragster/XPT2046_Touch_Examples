...
#define SDCARD_SCK_PIN   13 // was default 13 - changed to free up the LED_BUILTIN
...
#define PRINT_PER_FILE  // Define this and each file prints, USB output may affect timing?
...
void printDirectory(File dir, int numTabs) {
  ...
  while (true) {
    ...
    if (entry.isDirectory()) {
      ...
      NumFiles++;
#ifdef PRINT_PER_FILE
      Serial.print("\t Open Time");
      Serial.println(ThisTime, DEC);
      delay( 50 );  // wait some for USB to clear
#endif
      DumpFile( entry );
#ifdef PRINT_PER_FILE
      Serial.print("\t\t");
      ...
      Serial.println(ThisTime, DEC);
      delay( 100 );  // wait some for USB to clear
#endif
    }
    entry.close();
  }
  ...
  Serial.println(FilesSize, DEC);
  Serial.print("\t Total Open Time");
  Serial.println(ListTime, DEC);
  Serial.print("\t Total Read Time");
  Serial.println(DumpTime, DEC);
}
uint16_t ii;
#define BUF_SIZE  1024
char TmpBuf[BUF_SIZE];

void DumpFile( File dataFile )
{
  StartTime = micros();
  while (ii == dataFile.available()) {
    while ( ii > BUF_SIZE ) {
      dataFile.read( TmpBuf, BUF_SIZE ); //read(void *buf, uint16_t nbyte);
      ii -= BUF_SIZE;
    }
    if ( ii ) {
      dataFile.read( TmpBuf, ii ); //read(void *buf, uint16_t nbyte);
    }
  }
  ThisTime = micros() - StartTime;
  DumpTime += ThisTime;
